...
5. 부하 테스트 : k6 등의 도구로 동시 요청 시 시스템 안정성과 병목 구간을 분석해 최적화 포인트 도출(완료) 
6. 알림 시스템 MVP: `DUE_NOW` 발생 시 이메일 알림을 보내 실제 행동 유도(완료) 
7. 인사이트 재구성 : 인사이트 API를 의미 있는 지표 중심으로 재구성해 사용자에게 더 직관적이고 행동 가능한 정보 제공
8. 부하테스트 재시도 및 최적화 : k6 스크립트 개선과 테스트 반복으로 시스템의 최대 처리량과 안정성 한계 탐색, API 최적화 진행 
9. 오류 수정 및 리팩토링 : 운영 중 발견된 버그 수정과 코드 개선으로 시스템 안정성 및 유지보수성 강화
10. Grafana Alerting 고도화: `API down`/`5xx`/`p95` 임계치 알림 규칙과 Slack·Discord·Email 연동, 재알림/소음 제어 정책까지 포함해 운영 대응 자동화 강화
