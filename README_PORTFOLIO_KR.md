...
| 성능 검증 | k6 Read Matrix(최대 80 VU)에서 실패율 0%, p95 10ms 이내 |
...
실행 일시: **2026-05-12** (KST)  
시나리오: 인증 포함 Read Matrix (`5→20→40→60→80 VU`)

- 총 요청 수: `460,367`
- 최고 처리량: `553.22 req/s` (`80 VU`)
- 실패율(`http_req_failed`): `0.00%`
- checks 성공률: `100.00%`
- 전체 응답시간: `avg 4.14ms / p95 8.11ms / p99 12.17ms` (`80 VU`)
- 결론: read API 기준 `80 VU` 구간에서 성능/안정성 지표 모두 목표 범위 내 유지

### 9.3 엔드포인트 p95 (80 VU 기준, ms)

| API | p95(ms) | 목표 | 결과 |
|---|---:|---:|---|
| `GET /api/tasks?status=DUE_NOW` | 8.52 | < 800 | PASS |
| `GET /api/tasks?status=UPCOMING` | 8.42 | < 800 | PASS |
| `GET /api/insights/dashboard` | 8.87 | < 1000 | PASS |
| `GET /api/insights/overview` | 8.67 | < 1000 | PASS |
| `GET /api/insights/recent-completions` | 8.31 | < 1000 | PASS |

![k6 local read matrix grafana](infra/load/results/local-read-matrix-20260512-102647/grafana-dashboard-20260512.png)
