...
import com.yegkim.task_reloader_api.auth.dto.UpdateUserStatusRequest;
...
public class AdminUserController {
    ...
    @Operation(summary = "승인/거절 사용자 상태 변경 (APPROVED <-> REJECTED)")
    @PatchMapping("/{userId}/status")
    public ApiResponse<PendingUserResponse> updateNonPendingUserStatus(
            @PathVariable Long userId,
            @Valid @RequestBody UpdateUserStatusRequest request,
            @AuthenticationPrincipal AuthenticatedUser principal
    ) {
        return ApiResponse.success(authService.updateNonPendingUserStatus(principal.userId(), userId, request.status()));
    }
